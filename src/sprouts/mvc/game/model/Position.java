...
import sprouts.mvc.game.model.move.RawMove;
...
public class Position {
	...
	public Sprout createSprout(Vertex vertex) {
		...
		sprout.id = sproutIdGenerator.generate();
	...
	private RawMove getTwoBoundaryMove(Sprout from, Sprout to, Region region, Line line) {
		...
		if (isFromOuterBoundary || isToOuterBoundary) {
			region.setOuterBoundary(e1_1);
		} else {
			region.addInnerBoundary(e1_1);
		}
	...
	private RawMove getOneBoundaryMove(Sprout from, Sprout to, Region region, Line line) {
		...
		if (isSoloSprout) {
			newRegion.setOuterBoundary(e2_2);
			region.addInnerBoundary(e1_1);
		...
		List<Vertex> newRegionOuterBoundary = newRegion.getOuterBoundary().getBoundaryLine();
		List<Sprout> newRegionOuterSprouts = newRegion.getOuterBoundary().getBoundarySprouts();
		...
		innerSprouts.removeAll(transferredSprouts);
		innerBoundaries.removeAll(transferredBoundaries);
		
		List<Sprout> newRegionInnerSprouts = newRegion.getInnerSprouts();
		List<Edge> newRegionInnerBoundary = newRegion.getInnerBoundaries();
		
		newRegionInnerSprouts.addAll(transferredSprouts);
		newRegionInnerBoundary.addAll(transferredBoundaries);
		
		Edge outerBoundary = newRegion.getOuterBoundary();
		outerBoundary.setBoundaryRegion(newRegion);
	...
	private void addInnerToMove(RawMove move, Region region) {
		for (Edge innerBoundary : region.getInnerBoundaries()) {
		...
		for (Sprout innerSprout : region.getInnerSprouts()) {
	...
	private Region getLineRegion(Line line) {
		...
		for (int i = 0; i < regions.size(); i++) {
			...
			List<Vertex> outer = region.getOuterBoundary().getBoundaryLine();
			if (GraphicalFacade.isPointInPolygon(point, outer))	return region;
		}
