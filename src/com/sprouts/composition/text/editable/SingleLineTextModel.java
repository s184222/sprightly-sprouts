 ...
 * compositions such as the {@link TextFieldComposition}. This model ensures
 * that characters which represent a new line will be discarded. Doing this
 * ensures that the model will always stay single-lined. The new-line characters
 * that are considered by this model are the following:
 * <ul>
 ...
 * above, it is possible to extend the {@code SingleLineTextModel} and override
 ...
 * that are inserted:
 * <pre>
 * public class NoWhitespaceTextModel extends SingleLineTextModel {
 * 
 *     {@literal @}Override
...
public class SingleLineTextModel extends AbstractTextModel {
	...
	public SingleLineTextModel(int initialCapacity) {
	 ...
	 * {@link com.g4mesoft.composition.text.editable.SingleLineTextModel doc}.
	 * 
	...
	public void insertText(int offset, String text) {
		if (offset < 0 || offset > length)
			throw new TextModelIndexOutOfBoundsException(offset);
		
		int count = text.length();
		
		// Check number of characters that should not be
		// included when inserting the text.
		int n = count;
	...
	public void removeText(int offset, int count) {
		...
		if (end > length)
			throw new TextModelIndexOutOfBoundsException(length);
	...
	public String getText(int offset, int count) {
		if (offset == 0 && count == length) {
			// Usually a user of the TextFieldComposition
			// will want to retrieve the entire model at
			// once. We cache the model in a string to
			// save allocations
			if (cacheText == null)
